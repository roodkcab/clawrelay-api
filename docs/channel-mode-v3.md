...
## 6. 全集群部署 + 文件夹信任坑（2026-06-15，重要）

V3 已铺到**全部 8 个订阅版 relay-claude**（生产 claude01-05 chroot + claude07-09 nspawn）。**仅适用订阅版 claude**；**排除** claude06=minimax（API key 计费，V3 订阅计费无意义、交互+channels 未必兼容 MiniMax 后端）、claude10/11=codex（不同二进制 relay-codex）、QA。
...
- nspawn：`nsenter -t <Leader> -m -u -i -n -p -- su - <u> -c '... /home/<u>/.local/bin/claude_openai_api --port <p> --mode=channelv3 ...'`
- 重启前置检查 + 逐实例验证 `"mode":"channelv3"`，失败即止。
