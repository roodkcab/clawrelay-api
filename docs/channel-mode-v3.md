...
>
> **当前生产 scope（2026-06-16 起，长期混合拓扑）**：channelv3 现仅用于 **claude01(50008) / claude02(50009)** 两个试点实例；其余订阅版 claude（claude03-05、claude07-09）已运行在更简单、已验证的 **V1**（旧称 legacy，`-p`/headless；启动标志为默认裸 `--port=`（等价 `--mode=v1`，旧值 `--mode=legacy` 仍兼容），`/health` 返回 `"mode":"v1"`）模式。背景：V3 当初是为解决 headless `-p` 模式不能走订阅计费；2026-06-16 Anthropic 公告**恢复了 `-p` 模式的订阅计费**，故大多数订阅实例稳定回到 V1，仅保留 claude01/02 作为 V3 试点。8 台订阅实例**共享同一份 relay-claude 2.0.0 二进制**，混版靠**启动标志**区分（见 §6.3），不是不同二进制。本文档作为 channelv3 的权威设计参考保留。
...
`--mode=channelv3`（与 V1 / V2(channel) 并存，互不影响）。
...
> **拓扑（2026-06-16 起，长期混合）**：channelv3 仅用于 **claude01(50008) / claude02(50009)** 两个试点实例（均 chroot）。其余订阅版 claude——claude03(50010)/claude04(50011)/claude05(50012)（chroot）+ claude07(50014)/claude08(50015)/claude09(50016)（nspawn）——运行 **V1** 模式（裸 `--port=<p>`，`claude -p` headless，新公告后仍计入订阅，`/health` 返回 `"mode":"v1"`）。**排除** claude06=minimax(50013) v1.1.4 V1（API key 计费，V3 订阅计费无意义、交互+channels 未必兼容 MiniMax 后端）、claude10/11=codex(50017/50018)（不同二进制 relay-codex）、QA(50009 2.0.0 V1)。
>
> 下面 §6.1–§6.5 的前置条件、信任坑、启动命令均针对 **channelv3 实例（claude01/02）**；V1 实例不需要 bridge/bun/folder-trust，启动只用裸 `--port=`（见 §6.3）。
...
**channelv3 实例（claude01/02，chroot）** —— 必须带 `--mode=channelv3` 全套 flag（裸 `--port=` 会静默回退 V1）：
...
**V1 实例（claude03-05 chroot / claude07-09 nspawn）** —— 裸 `--port=<p>`，无 channelv3 flag、无需 bridge/bun/folder-trust：
...
## 7. 稳定性加固（relay-claude 2.1.0，2026-07-05）

对抗审查(两轮多智能体审查+验证)后落地的一批 V3 稳定性修复,行为语义变化如下:

### 7.1 同 session turn 串行化(turnSlot)
V3 此前没有 V2 turnMu 的等价物:同 sid 并发的第二个请求会把消息 enqueue 给还在忙的 claude,自己等不到任何带自己 req_id 的事件,**5 分钟后被冷启动 watchdog 误判为冻死,反手 SIGKILL 掉正在服务上一轮的进程**(上一轮被静默截断+上下文全丢,排队消息被 drainInbox 连带丢弃,还会级联重杀)。现在每个 `v3Session` 持有容量 1 的 `turnSlot` 令牌:持有令牌才能 enqueue/等回复;排队请求等待期间发**可见的排队心跳**(`⏳ 同会话上一条消息仍在处理,排队中…`),且等待感知 ctx(客户端断开即放弃)。串行化后,watchdog 的"5 分钟零输出"只剩"会话真冻死"一种语义,误杀路径消除。

### 7.2 acquire 单飞 + inbox 生命周期
- `v3Manager.acquire` 现在按 sid 单飞(同 V2 A8):并发首请求不再各自 launch 出两个交互式 claude(旧行为:败者被 map 覆盖成永久泄漏的重进程,两个 bridge 抢同一 inbox)。
- inbox 与 session 同生共死:促升时创建,killSession/reap/evict/onSessionDie/Stop 全部同步删除;`/v3/next` 对未知 sid 返回 404 **不再自建条目**(旧行为:被拆会话的 bridge 最后一次 poll 会在 map 里留永久条目,且残留旧消息会重放进同 sid 的下一个会话)。
- `enqueue` 带 ctx + 30s 逃逸,不再可能把 handler goroutine 永久吊死在满 inbox 上。
- 客户端断连(stop)后 drainInbox:自己那条还没被 bridge 取走的消息不再污染下一轮。
- 冷启动 give-up 后**拆掉冻死会话**(旧行为:留在 manager 里,下一条请求原样再白等 5 分钟)。

### 7.3 进程与文件防护
- `kill()` 带 reaped 防护:进程已被 cmd.Wait 收割后不再裸 kill(-pid),防 PID 复用误杀无关进程组。
- `.mcp.json`:全局锁防并发读改写互相丢更新;既有文件是非法 JSON 时**拒绝启动并报错**而不是静默清空(可能是用户手写配置);写入原子化(tmp+rename);PTY 启动失败不再顺手删除这个共享文件。
- `Stop()`(SIGTERM)先摘表再逐会话 flushV3Orphan+kill:重启不再丢未收割的尾巴 token。

### 7.4 bridge.ts 加固
- 所有工具转发 POST 带 15s 超时;`reply`(终态)失败重试 3 次(1s/2s/4s 退避),全败 loud 日志但仍放行 pump(绝不卡死)。
- 注入(`mcp.notification`)失败重试 5 次——消息已离开 relay inbox,丢了只能靠 5 分钟 watchdog 兜底,重试把这个窗口基本堵死。
- pump 串行化兜底从**固定 20 分钟**改为**活动感知**(progress/reply_chunk/reply 都重置静默计时,静默 20min 或绝对 2h 先到者放行)——旧行为会在长任务(claude 每分钟发 progress、relay 还在正常等)进行到 20 分钟时把排队消息硬注入还在忙的 claude,正是串行化要防的双消息挂死。
- `uncaughtException` 记日志后退出(半死 bridge 持续吞消息比死掉更糟;relay watchdog 会兜底)。

### 7.5 已知残留(有意不做/待观察)
- V3 会话被 idle 回收/驱逐后 launch 不带 `--resume`,**多轮上下文清零**(用户体感"过了一会儿就失忆")。这是 Area B transcript 计量的有意设计(复用 uuid 会导致 claude 拒启或意外续话),要恢复上下文需设计 resume+offset 续读方案并实测订阅计费不受影响——单独立项,勿顺手改。
- bridge 2h 绝对上限触发时仍可能向忙碌 claude 双注入(防永久卡死的有界代价)。
- 测试环境无 gcc,本批改动未过 -race(逻辑上已穷举审查锁生命周期)。
